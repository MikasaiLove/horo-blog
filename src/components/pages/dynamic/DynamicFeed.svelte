...
import { registerDynamicInlineComments } from "./dynamic-inline-comments";
	import { initAllDynamicViews } from "./dynamic-views";
...
function createItem(entry: DynamicData) {
	...
	if (content) {
		...
		if (gallery) gallery.dataset.sourceId = content.id;
			...
			const pvSpan = root.querySelector<HTMLElement>(".dy-pv-text");
			if (pvSpan) pvSpan.dataset.path = "/dynamic/" + entry.id.replace(/.md$/, "");
